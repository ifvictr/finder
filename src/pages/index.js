...
class Index extends Component {
  ...
  render() {
    ...
    const CurrentSearchInput = searchByLocation
      ? LocationSearchInput
      : SearchInput
    ...
    return (
      <Fragment>
        ...
        <Flex
          justify={hasResults ? 'initial' : 'center'}
          style={{ margin: -theme.space[2], marginTop: theme.space[2] }}
          wrap
        >
          {filteredClubs.map(club => (
            <LazyLoad key={club.id} height={0} offset={100} once overflow>
              <ClubCard
                data={club}
                distance={
                  searchByLocation && searchLat && searchLng
                    ? geolib.getDistance(
  ...
  getClosestResult(results) {
    return (
      results.find(result => result.types.includes('neighborhood')) ||
      results[0]
