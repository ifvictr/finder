...
let defaultUnit="Imperial",defaultDistance="mile";function updateResultCount(t){const a=`${t} club${1===t?"":"s"}`;$("[data-output='total-results']").html(a)}function updateResults(t,a,e=!1){const s=$("#results");s.empty(),e||(t=geolib.orderByDistance(a,t).filter(t=>getDistance(t.distance)<parseInt($("input[name='radius']").val()))),updateResultCount(t.length);const n=!!t.length;$("#results").toggleClass("is-hidden",!n),$("#no-results").toggleClass("is-hidden",n),t.map(t=>{const a=getDistance(t.distance).toFixed(1),n=`assets/images/school/${t.id}.jpg`;s.append(`\n            <div class="column is-3" data-club-id="${t.id}">\n                <div class="card">\n                    <div class="card-image">\n                        <figure class="image is-3by2">\n                            <img src="assets/images/placeholder.svg">\n                        </figure>\n                    </div>\n                    <div class="card-content">\n                        <h2 class="title is-5 is-spaced">${t.name}</h2>\n                        <h3 class="subtitle is-6">${t.address}</h3>\n                        ${e?"":`<span>${a>1?a+" "+defaultDistance+"s":"<1 "+defaultDistance} away</span>`}\n                    </div>\n                    <footer class="card-footer">\n                        \x3c!-- <a href="#" target="_blank" class="card-footer-item" title="Visit club website"><span class="icon"><i class="fa fa-link"></i></span></a> --\x3e\n                        <a href="https://www.google.com/maps/place/${t.name+", "+t.address}" target="_blank" class="card-footer-item" title="View on Google Maps"><span class="icon"><i class="fa fa-map"></i></span></a>\n                        \x3c!-- <a href="#" target="_blank" class="card-footer-item" title="Reach out"><span class="icon"><i class="fa fa-comment"></i></span></a> --\x3e\n                    </footer>\n                </div>\n            </div>\n        `),isSuccess(n,()=>{const a=$(`[data-club-id="${t.id}"] .image`);a.find("img").attr("src",n),a.css("opacity",1)})})}$(()=>{let t=[],a=[],e=null;$.ajax({url:"https://api.hackclub.com/v1/clubs",type:"get",dataType:"json"}).done(s=>{t=s.sort(byField("name")),e=new Fuse(t,{shouldSort:!0,threshold:.3,location:0,distance:100,maxPatternLength:32,minMatchCharLength:3,keys:["name","address"]}),navigator.geolocation.getCurrentPosition(e=>{a={latitude:e.coords.latitude,longitude:e.coords.longitude},$.ajax({url:`https://maps.google.com/maps/api/geocode/json?latlng=${a.latitude+","+a.longitude}`,type:"get",dataType:"json"}).done(t=>{if(t.results.length>0){const a=t.results[0];updateUnit(t),$("[data-action='search-nearby']").val(a.formatted_address)}}).fail(console.log),updateResults(t,a),$("[data-output='location']").html("near you")})}).fail(console.log),$("input[data-action='search-nearby']").on("input",function(){const e=$(this).val();0!==e.length&&setTimeout(()=>{const s=$("input[data-action='search-nearby']").val();e===s&&$.ajax({url:`https://maps.google.com/maps/api/geocode/json?address=${encodeURI(s)}`,type:"get",dataType:"json"}).done(e=>{if(e.results.length>0){const s=e.results[0],n=s.geometry.location;a={latitude:n.lat,longitude:n.lng},updateUnit(e),updateResults(t,a),$("[data-output='location']").html("near "+s.formatted_address)}else updateResults([],a)}).fail(console.log)},1250)}),$("input[data-action='search-all']").on("input",function(){const t=$(this).val();0!==t.length&&setTimeout(()=>{const s=$("input[data-action='search-all']").val();t===s&&updateResults(e.search(t),a,!0)},1250)}),$("input[type='range']").on("input",function(){const e=$(this).val();$("[data-output='radius']").html(e+" "+defaultDistance),setTimeout(()=>{const s=$("input[type='range']").val();e===s&&updateResults(t,a)},1250)}),$("#toggle-search-all").on("change",function(){$("[data-type='nearby']").toggleClass("is-hidden"),$("[data-type='base-unit']").toggleClass("is-hidden"),$("[data-type='all']").toggleClass("is-hidden"),$("[data-output='location']").toggleClass("is-hidden"),updateResults(t,a,this.checked)}),$("#toggle-unit").on("change",function(){let e=$("[for='toggle-unit']"),s=e.html();"Imperial"==s?(defaultUnit="Metric",defaultDistance="kilometer"):"Metric"==s&&(defaultUnit="Imperial",defaultDistance="mile"),e.html(defaultUnit);const n=$("input[type='range']").val();$("[data-output='radius']").html(n+" "+defaultDistance),updateResults(t,a)})});
const byField=e=>(t,i)=>{const s=t[e],l=i[e];return s<l?-1:s>l?1:0},isSuccess=(e,t)=>{$.ajax({url:e,statusCode:{200:t}})},updateUnit=e=>{let t;try{t=e.results[0].address_components.slice(-1)[0].short_name}catch(e){t="US"}-1!="US|MM|LR".indexOf(t)?(defaultUnit="Imperial",defaultDistance="mile"):(defaultUnit="Metric",defaultDistance="kilometer"),$("#toggle-unit").click()},toMiles=e=>621371e-9*e,toKilometers=e=>.001*e,getDistance=e=>"Imperial"==defaultUnit?toMiles(e):toKilometers(e);
